...
public class paymentDAOImp implements paymentDAO{
    ...
    public int save(payment p, int a_id) {
        String sql = "insert into payment(account_id, bank_name, card_number, fullname) values(?,?,?,?)";
        return jt.update(sql, a_id, p.getBank_name(), p.getCard_number(), p.getFname());
    }
            ...
            public payment extractData(ResultSet rs){
                try {
                    if(rs.next())
                    {
                        ...
                        p.setCard_number(rs.getInt("card_number"));
